    ...
    (function() {
      ...
      function handleInitData(data) {
        ...
        try {
          ...
          // A history prepend keeps the previously latest candle in the dataset.
          // If it's gone, this is a fresh dataset (e.g. an interval switch) and the
          // old visible range no longer applies, so we re-fit instead of preserving it.
          var isContinuation = previousLastTime !== null && formattedCandles.some(function(c) {
            return c.time === previousLastTime;
          });
          var barsPrepended = 0;
          if (isContinuation && previousFirstTime !== null) {
